...
export interface Match {
  ...
  matchNumber: string;
  matchType: string;
  matchDate: string; // Change to string to store in YYYY-MM-DD format
  matchTime: string;
  location: string;
  tournamentId: string;
  firstTeamId: string;
...
const CreateMatch: React.FC = () => {
  ...
  const [matchData, setMatchData] = useState<Match>({
    ...
    matchNumber: '',
    matchType: '',
    matchDate: new Date().toISOString().split('T')[0], // Initialize as a formatted string
    matchTime: '',
    location: '',
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      ...
      try {
        ...
        console.log(response.data); // Assuming response contains an array of teams
      } catch (error) {
